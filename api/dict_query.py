...
async def dict_query(request: Request):
    ...
    try:
        url_list = await get_all_url(request.word)
        if not url_list:
            return Response(
                status=404,
                result=None,
        ...
        results = await get_dict(url_list)
        return Response(
            status=200,
            result=results,
            error=None
...
if __name__ == "__main__":
    async def test():
        print(await dict_query(Request(word="先生")))
        print(await dict_query(Request(word="少女")))
        print(await dict_query(Request(word="食べる")))
        print(await dict_query(Request(word="嗨嗨")))
